...
std::pair<int, std::vector<int>> knapsack(int capacity, const std::vector<int>& weights, const std::vector<int>& values) {
    ...
    // Retrieve the weights of the items included in the knapsack
    std::vector<int> included_weights;
    int i = weights.size(), j = capacity;
    while (i > 0 && j > 0) {
        if (keep[i][j] == 1) {
            included_weights.push_back(weights[i - 1]);
            j -= weights[i - 1];
        }
        i--;
    }
    return {table[weights.size()][capacity], included_weights};
}
